/**
 * This file integrates the hc-sticky plugin with the Chameleon1 skin
 *
...
;( function (window, document, $, mw, undefined) {
	...
	$( function () {
		...
		$( 'body' ).scrollspy( { target: '.chameleon1-toc', offset: offset } );

		function goToLink( $link ) {
			var $activeLink = $( '.chameleon1-toc .active' );
		...
		function setInitialLink() {
			...
			var activeLink = $('.chameleon1-toc a.active');
			var targetLink;

			if ( hash === '' && activeLink.length === 0) {
				targetLink = $( '.chameleon1-toc a.top' );
			} else {
				targetLink = $( '.chameleon1-toc a[href="' + hash + '"]' );
			}
		...
		$( window ).on( 'scroll', function() {
			$( '.chameleon1-toc a.clicked' ).removeClass( 'clicked' );
			...
			goToLink( $( '.chameleon1-toc a.top' ) );
		} );

		$( '.chameleon1-toc ul li a').on( 'click', function () {
			...
			$( '.chameleon1-toc ul li a').removeClass( 'clicked' );
		...
		$( window ).on( 'activate.bs.scrollspy', function ( e, obj ) {
			var clickedLink = $( '.chameleon1-toc .clicked' );
