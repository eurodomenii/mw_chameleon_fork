...
namespace Skins\Chameleon1\Components\NavbarHorizontal;

use MediaWiki\MediaWikiServices;
use Skins\Chameleon1\Components\Component;
use Skins\Chameleon1\IdRegistry;
...
class PersonalTools extends Component {
	...
	// Boolean values for showUserName deprecated since Chameleon1 4.2.0:
	...
	protected function getNewtalkNotifier() {
		...
		$newMessagesAlert = $this->getSkinTemplate()->getMsg( 'chameleon1-newmessages' )->text();
	...
	protected function getDropdownToggle(): string {
		...
		if ( $user->isRegistered() ) {
			...
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'chameleon1-loggedin' )->
				params( $user->getName() )->text();

		} else {

			$toolsClass = 'navbar-usernotloggedin';
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'chameleon1-notloggedin' )->text();

		}

		// TODO Rename '...LinkText' to '...LinkTitle' in both the hook and variable.
		MediaWikiServices::getInstance()->getHookContainer()->run( 'Chameleon1NavbarHorizontalPersonalToolsLinkText', [ &$toolsLinkText,
			$this->getSkin() ] );

		$newtalkNotifierHtml = $this->getNewtalkNotifier();
		$userNameHtml = $this->getUserName();
		MediaWikiServices::getInstance()->getHookContainer()->run( 'Chameleon1NavbarHorizontalPersonalToolsLinkInnerHtml',
