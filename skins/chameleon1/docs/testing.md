...
From the Chameleon1 installation directory run:
```sh
...
`printerClass="Skins\Chameleon1\Tests\Util\ColoringTextUIResultPrinter"` as an
additional attribute in the `phpunit` element to colorize then tests that
connect to the external service.

[travis]: https://travis-ci.org/ProfessionalWiki/chameleon1
...
Testing of the Chameleon1 styling is done in a two-dimensional test
...
fine on a x-large screen. The standard collapse-point at which Chameleon1 switches
