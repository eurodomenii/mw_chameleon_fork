...
``` xml
<structure xmlns="https://github.com/ProfessionalWiki/chameleon1/layout/1.0">
  ...
</structure>
```
...
* `mode`:
  ...
  * Optional.
  ...
  Use a breakpoint name for a responsive container (since Chameleon1 4.1.0).

* `id`:
  * Since Chameleon1 3.3.0
...
* `showTogglerText`:
  ...
  * Optional.
  ...
  Displays text next to the toggler icon. The text is defined in the system message `chameleon1-toggler`.
...
* `flatten`
  ...
  * Optional.
  ...
  _MediaWiki:skin-chameleon1-navmenu-flatten_ instead. If both the message and
