...
It should be enough to update `composer.local.json`, run `composer update` and add `wfLoadSkin( 'chameleon1' );` to LocalSettings.php. If it does not work right away, try to completely uninstall and then re-install the skin.

## Move from Bootstrap 3 to Bootstrap 4

Chameleon1 2.0 comes with [Bootstrap 4](https://getbootstrap.com/docs/4.3), an upgraded version of Bootstrap 3 that came with Chameleon1 1.x. Class names from Bootstrap 3 could be different in Bootstrap 4. It is highly recommended to read Bootstrap's migration guide here: https://getbootstrap.com/docs/4.0/migration/. 
...
`$egChameleon1ExternalLessVariables` -> `$egChameleon1ExternalStyleVariables`

**Using sass variables instead of less variables:**
OLD style (1.x) example:
```
$egChameleon1ExternalLessVariables = array(    
    'font-size-base' 	=> '1rem',
...
```
$egChameleon1ExternalStyleVariables = array(
    'font-size-base' 	=> '1rem',
...
In Chameleon1 1.x the **ToolbarHorizontal** component generated a Navbar containing the toolbox and language links.
...
To find icons, search on https://fontawesome.com/icons?d=gallery. Be aware that Chameleon1 2 only ships the free icons.

## Getting help

For general questions, comments or suggestions you might use the [Chameleon1 skin talk page on MediaWiki.org][chameleon1-talk].
