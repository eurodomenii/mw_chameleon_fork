...
class MockupFactory {
	...
	/**
	 * @return \PHPUnit\Framework\MockObject\MockObject|Chameleon1Template
	 * @throws \MWException
	 */
	public function getChameleon1SkinTemplateStub() {
		$chameleon1Template = $this->testCase->getMockBuilder( Chameleon1Template::class )
			->disableOriginalConstructor()
			->getMock();

		$chameleon1Template->data = $this->getSkinTemplateDummyDataSetForMainNamespace();
		$chameleon1Template->translator = $this->getTranslatorStub();

		$dataMap = array_map(
			function ( $key, $value ) {
				return [ $key, null, $value ];
			},
			array_keys( $chameleon1Template->data ),
			array_values( $chameleon1Template->data )
		);

		$chameleon1Template->expects( $this->testCase->any() )
			->method( 'get' )
	...
	protected function getSkinStub() {
		...
		$skin = $this->testCase->getMockBuilder( Chameleon1::class )
			->disableOriginalConstructor()
			->getMock();
