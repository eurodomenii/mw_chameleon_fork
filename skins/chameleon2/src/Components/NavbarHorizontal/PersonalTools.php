...
use Skins\Chameleon2\IdRegistry;
...
class PersonalTools extends Component {
	...
	// Boolean values for showUserName deprecated since Chameleon2 4.2.0:
	...
	protected function getNewtalkNotifier() {
		...
		$newMessagesAlert = $this->getSkinTemplate()->getMsg( 'chameleon2-newmessages' )->text();
	...
	protected function getDropdownToggle(): string {
		...
		if ( $user->isRegistered() ) {
			...
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'chameleon2-loggedin' )->
				params( $user->getName() )->text();

		} else {

			$toolsClass = 'navbar-usernotloggedin';
			$toolsLinkText = $this->getSkinTemplate()->getMsg( 'chameleon2-notloggedin' )->text();

		}

		// TODO Rename '...LinkText' to '...LinkTitle' in both the hook and variable.
		MediaWikiServices::getInstance()->getHookContainer()->run( 'Chameleon2NavbarHorizontalPersonalToolsLinkText', [ &$toolsLinkText,
			$this->getSkin() ] );

		$newtalkNotifierHtml = $this->getNewtalkNotifier();
		$userNameHtml = $this->getUserName();
		MediaWikiServices::getInstance()->getHookContainer()->run( 'Chameleon2NavbarHorizontalPersonalToolsLinkInnerHtml',
