...
use Skins\Chameleon2\Components\Container;
use Skins\Chameleon2\Components\Structure;
...
class ComponentFactory {
	...
	private const NAMESPACE_HIERARCHY = 'Skins\\Chameleon2\\Components';
	...
	public function getLayoutXml(): string {
		...
		$this->hookContainer->run(
			Chameleon2::HOOK_GET_LAYOUT_XML,
			[ &$xml ]
		);
	...
	protected function getModifiedComponent( DOMElement $description, Component $component ) {
		...
		$className = 'Skins\\Chameleon2\\Components\\Modifications\\' .
			$description->getAttribute( 'type' );

		if ( !class_exists( $className ) || !is_subclass_of( $className,
			'Skins\\Chameleon2\\Components\\Modifications\\Modification' ) ) {
			throw new MWException(
	...
	protected function mapComponentDescriptionToClassName( DOMElement $description ): string {
		if ( $description->hasAttribute( 'type' ) ) {
			...
			$chameleon2ClassName = implode( '\\', [ self::NAMESPACE_HIERARCHY, $className ] );
			if ( class_exists( $chameleon2ClassName ) ) {
				return $chameleon2ClassName;
			}
