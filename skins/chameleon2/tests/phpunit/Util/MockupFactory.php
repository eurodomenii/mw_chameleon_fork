...
namespace Skins\Chameleon2\Tests\Util;
...
class MockupFactory {
	...
	/**
	 * @return \PHPUnit\Framework\MockObject\MockObject|Chameleon2Template
	 * @throws \MWException
	 */
	public function getChameleon2SkinTemplateStub() {
		$chameleon2Template = $this->testCase->getMockBuilder( Chameleon2Template::class )
			->disableOriginalConstructor()
			->getMock();

		$chameleon2Template->data = $this->getSkinTemplateDummyDataSetForMainNamespace();
		$chameleon2Template->translator = $this->getTranslatorStub();

		$dataMap = array_map(
			function ( $key, $value ) {
				return [ $key, null, $value ];
			},
			array_keys( $chameleon2Template->data ),
			array_values( $chameleon2Template->data )
		);

		$chameleon2Template->expects( $this->testCase->any() )
			->method( 'get' )
	...
	protected function getSkinStub() {
		...
		$skin = $this->testCase->getMockBuilder( Chameleon2::class )
			->disableOriginalConstructor()
			->getMock();
